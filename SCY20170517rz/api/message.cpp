...
int Message::messageProc(QString str,QextSerialPort *com,QTcpSocket *tcp)
{
    ...
    case CN_Adjust:
    {
        ...
        myCom[0]->write("reset 0\r\n");
        myCom[0]->flush();
        SwitchOut_On(6);
        sleep(3);
        QByteArray readbuf;
        readbuf=myCom[0]->readAll();
        if(readbuf=="sucess")ExecuteRespond(RESULT_SUCCESS,com,tcp);
        else ExecuteRespond(RESULT_FAILED,com,tcp);
        SwitchOut_Off(6);
    ...
    case CN_Control:
    {
        ...
        //添加控制设备
        myCom[0]->write("get  states\r\n");
        myCom[0]->flush();
        SwitchOut_On(9);
        sleep(3);
        QByteArray readbuf;
        readbuf=myCom[0]->readAll();
        if(readbuf=="working")ExecuteRespond(RESULT_SUCCESS,com,tcp);
        else   ExecuteRespond(RESULT_FAILED,com,tcp);
        SwitchOut_Off(9);
    ...
    case CN_SampleTime:
    {
        ...
        int count_number;
        sendbuf="set ctime ";
        for(count_number=1;count_number<str.split(",CTime=").count();count_number++)
        {
            sendbuf+=str.split(",CTime=")[count_number]+",";

        }
        sendbuf=sendbuf.left(sendbuf.length()-9);
        sendbuf+="\r\n";
        //添加设置采样间隔
