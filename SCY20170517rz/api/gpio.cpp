...
int InitGPIO()
{
    ...
    printf("open file = %d\n", fd);
    rc = GPIO_OutEnable(fd,GPIO0|SWITCH_OUT1|SWITCH_OUT2|SWITCH_OUT3|SWITCH_OUT4|SWITCH_OUT5|SWITCH_OUT6|SPI_CS);//set GPIO as output
    if(rc < 0)
    {
        printf("GPIO_OutEnable::failed %d\n", rc);
    }

    rc = GPIO_OutDisable(fd,SWITCH_IN1|SWITCH_IN2|SWITCH_IN3|SWITCH_IN4|SWITCH_IN5|SWITCH_IN6);   //set GPIO as input
    if(rc < 0)
    {
